...
def get_logger(
    name: str, log_level: str | int = DEFAULT_LOG_LEVEL
) -> logging.Logger:
    log_level = log_level
...
class LightningLoggingConfig:
    """Credits: https://github.com/CLARIN-PL/embeddings."""

    loggers_names: list[Literal["wandb", "csv", "tensorboard"]] = field(
    ...
    def __post_init__(self) -> None:
        if "wandb" not in self.loggers_names and (
            self.tracking_project_name
            or self.wandb_entity
            or self.wandb_logger_kwargs
        ):
            raise ValueError(
    ...
    def get_lightning_loggers(
        ...
        if self.use_csv():
            name = run_name if run_name else "lightning_logs"
            loggers.append(
                pl_loggers.CSVLogger(
                    name=name, save_dir=str(output_path.joinpath("csv"))
                )
            )
...
class ExperimentLogger(abc.ABC):
    @abc.abstractmethod
    def log_output(
        self, output_path: T_path, run_name: Optional[str] = None
    ) -> None:
        pass
    ...
    @abc.abstractmethod
    def log_artifact(
        self, paths: Iterable[T_path], artifact_name: str, artifact_type: str
    ) -> None:
        pass
